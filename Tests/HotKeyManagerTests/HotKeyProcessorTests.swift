...
struct HotKeyProcessorTests {
    ...
    func pressAndHold_startsRecordingOnHotkey_standard() throws {
        runScenario(
            ...
            steps: [
                ScenarioStep(time: 0.0, key: .a, modifiers: [.command], expectedOutput: .keyDown, expectedIsMatched: true),
            ]
        )
    }
    ...
    func pressAndHold_startsRecordingOnHotkey_modifierOnly() throws {
        runScenario(
            ...
            steps: [
                ScenarioStep(time: 0.0, key: nil, modifiers: [.option], expectedOutput: .keyDown, expectedIsMatched: true),
            ]
        )
    }
    ...
    func pressAndHold_stopsRecordingOnHotkeyRelease_standard() throws {
        runScenario(
            ...
            steps: [
                ScenarioStep(time: 0.0, key: .a, modifiers: [.command], expectedOutput: .keyDown, expectedIsMatched: true),
                ScenarioStep(time: 0.2, key: nil, modifiers: [.command], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    
    // Tests partial release (holding key, releasing modifier) does NOT trigger keyUp yet
    @Test
    func pressAndHold_partialRelease_holdsState() throws {
        runScenario(
            hotkey: HotKey(key: .a, modifiers: [.command]),
            steps: [
                // Press Cmd+A
                ScenarioStep(time: 0.0, key: .a, modifiers: [.command], expectedOutput: .keyDown, expectedIsMatched: true),
                // Release Cmd, keep A held -> Should NOT trigger keyUp yet
                ScenarioStep(time: 0.2, key: .a, modifiers: [], expectedOutput: nil, expectedIsMatched: true),
                // Release A -> NOW trigger keyUp
                ScenarioStep(time: 0.3, key: nil, modifiers: [], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    
    // Tests partial release (holding modifier, releasing key) does NOT trigger keyUp yet
    @Test
    func pressAndHold_partialRelease_holdsState_modifierHeld() throws {
        runScenario(
            hotkey: HotKey(key: .a, modifiers: [.command]),
            steps: [
                // Press Cmd+A
                ScenarioStep(time: 0.0, key: .a, modifiers: [.command], expectedOutput: .keyDown, expectedIsMatched: true),
                // Release A, keep Cmd held -> Should NOT trigger keyUp yet
                ScenarioStep(time: 0.2, key: nil, modifiers: [.command], expectedOutput: nil, expectedIsMatched: true),
                // Release Cmd -> NOW trigger keyUp
                ScenarioStep(time: 0.3, key: nil, modifiers: [], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func pressAndHold_stopsRecordingOnHotkeyRelease_modifierOnly() throws {
        runScenario(
            ...
            steps: [
                ScenarioStep(time: 0.0, key: nil, modifiers: [.option], expectedOutput: .keyDown, expectedIsMatched: true),
                ScenarioStep(time: 0.2, key: nil, modifiers: [], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func pressAndHold_stopsRecordingOnHotkeyRelease_multipleModifiers() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.1, key: nil, modifiers: [.option, .command], expectedOutput: .keyDown, expectedIsMatched: true),
                ScenarioStep(time: 0.2, key: nil, modifiers: [], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func pressAndHold_releasingModifierBeforeKeyStillStops() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.05, key: .u, modifiers: [.option], expectedOutput: .keyDown, expectedIsMatched: true),
                // Release the modifier while holding the key -> Partial release, should hold
                ScenarioStep(time: 1.5, key: .u, modifiers: [], expectedOutput: nil, expectedIsMatched: true),
                // Release the key a beat later -> NOW stop
                ScenarioStep(time: 1.55, key: nil, modifiers: [], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func pressAndHold_cancelsOnOtherKeyPress_standard() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.5, key: .b, modifiers: [.command], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func pressAndHold_doesNotTriggerOnBackslide_standard() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.3, key: .a, modifiers: [.command], expectedOutput: .keyDown, expectedIsMatched: true),
            ]
        )
    }
    ...
    func doubleTapLock_startsRecordingOnDoubleTap_standard() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.0, key: .a, modifiers: [.command], expectedOutput: .keyDown, expectedIsMatched: true),
                // First release
                ScenarioStep(time: 0.1, key: nil, modifiers: [.command], expectedOutput: .keyUp, expectedIsMatched: false),
                ...
                // Second tap within threshold -> doubleTapDown
                ScenarioStep(time: 0.2, key: .a, modifiers: [.command], expectedOutput: .doubleTapDown, expectedIsMatched: true),
                // Second release (should stay recording) -> doubleTapUp
                ScenarioStep(time: 0.3, key: nil, modifiers: [.command], expectedOutput: .doubleTapUp, expectedIsMatched: true, expectedState: .doubleTapLock),
            ]
        )
    }
    ...
    func doubleTapLock_startsRecordingOnDoubleTap_modifierOnly() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.0, key: nil, modifiers: [.option], expectedOutput: .keyDown, expectedIsMatched: true),
                // First release
                ScenarioStep(time: 0.1, key: nil, modifiers: [], expectedOutput: .keyUp, expectedIsMatched: false),
                // Second tap within threshold
                ScenarioStep(time: 0.2, key: nil, modifiers: [.option], expectedOutput: .doubleTapDown, expectedIsMatched: true),
                // Second release (should stay recording)
                ScenarioStep(time: 0.3, key: nil, modifiers: [], expectedOutput: .doubleTapUp, expectedIsMatched: true, expectedState: .doubleTapLock),
            ]
        )
    }
    ...
    func doubleTapLock_startsRecordingOnDoubleTap_multipleModifiers() throws {
        runScenario(
            ...
            steps: [
                // First tap
                ScenarioStep(time: 0.0, key: nil, modifiers: [.option], expectedOutput: nil, expectedIsMatched: false),
                ScenarioStep(time: 0.05, key: nil, modifiers: [.option, .command], expectedOutput: .keyDown, expectedIsMatched: true),
                // First release
                ScenarioStep(time: 0.1, key: nil, modifiers: [.option], expectedOutput: .keyUp, expectedIsMatched: false),
                // Second tap within threshold
                ScenarioStep(time: 0.2, key: nil, modifiers: [.option, .command], expectedOutput: .doubleTapDown, expectedIsMatched: true),
                // Second release (should stay recording)
                ScenarioStep(time: 0.3, key: nil, modifiers: [.option], expectedOutput: .doubleTapUp, expectedIsMatched: true, expectedState: .doubleTapLock),
            ]
        )
    }
    ...
    func doubleTapLock_ignoresSlowDoubleTap_standard() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.4, key: .a, modifiers: [.command], expectedOutput: .keyDown, expectedIsMatched: true),
            ]
        )
    }
    ...
    func doubleTapLock_ignoresSlowDoubleTap_modifierOnly() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.1, key: nil, modifiers: [], expectedOutput: .keyUp, expectedIsMatched: false),
                // Second tap after threshold
                ScenarioStep(time: 0.4, key: nil, modifiers: [.option], expectedOutput: .keyDown, expectedIsMatched: true),
            ]
        )
    }
    ...
    func doubleTapLock_stopsRecordingOnNextTap_standard() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 1.0, key: .a, modifiers: [.command], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func doubleTapLock_stopsRecordingOnNextTap_modifierOnly() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.3, key: nil, modifiers: [], expectedOutput: .doubleTapUp, expectedIsMatched: true, expectedState: .doubleTapLock),
                // Third tap to stop recording
                ScenarioStep(time: 1.0, key: nil, modifiers: [.option], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func modifierOnly_fn_triggersAfterFnPlusKeyThenFullRelease() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.20, key: nil, modifiers: [.fn], expectedOutput: .keyDown, expectedIsMatched: true),
                // Release Fn should stop recording (must exceed modifierOnlyMinimumDuration)
                ScenarioStep(time: 0.40, key: nil, modifiers: [],    expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func modifierOnly_fn_doesNotTriggerWhenFnRemainsHeldAfterKeyRelease() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.25, key: nil, modifiers: [.fn], expectedOutput: .keyDown, expectedIsMatched: true),
                // Must exceed modifierOnlyMinimumDuration before stopping
                ScenarioStep(time: 0.60, key: nil, modifiers: [],    expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func pressAndHold_staysDirtyAfterTwoSeconds() throws {
        runScenario(
            ...
            steps: [
                ...
                // Release option
                ScenarioStep(time: 2.2, key: nil, modifiers: [], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func doubleTap_onlyLocksAfterSecondRelease() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.2, key: nil, modifiers: [.option], expectedOutput: .doubleTapDown, expectedIsMatched: true, expectedState: .pressAndHold(startTime: Date(timeIntervalSince1970: 0.2), isDoubleTap: true)),
    ...
    func doubleTap_secondTapHeldTooLongBecomesHold() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 2.3, key: nil, modifiers: [], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func escape_cancelsFromLock() throws {
        runScenario(
            ...
            steps: [
                ...
                // Second tap (locks)
                ScenarioStep(time: 0.2, key: nil, modifiers: [.option], expectedOutput: .doubleTapDown, expectedIsMatched: true),
                ScenarioStep(time: 0.3, key: nil, modifiers: [], expectedOutput: .doubleTapUp, expectedIsMatched: true),
    ...
    func escape_whileHoldingHotkey_doesNotRestart() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.8, key: .a, modifiers: [.command], expectedOutput: .keyDown, expectedIsMatched: true),
            ]
        )
    }
    ...
    func modifierOnly_doesNotTriggerWithOtherKeys() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.3, key: nil, modifiers: [.command, .option], expectedOutput: .keyDown, expectedIsMatched: true),
                // Release
                ScenarioStep(time: 0.4, key: nil, modifiers: [], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func multipleModifiers_partialRelease() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.0, key: nil, modifiers: [.option, .command], expectedOutput: .keyDown, expectedIsMatched: true),
                // Release Command (keep Option) - should stop recording
                ScenarioStep(time: 0.5, key: nil, modifiers: [.option], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func keyModifier_changingModifiers_cancelsWithin1s() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.5, key: .a, modifiers: [.command, .shift], expectedOutput: .keyUp, expectedIsMatched: false),
            ]
        )
    }
    ...
    func dirtyState_blocksInputUntilFullRelease() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.5, key: nil, modifiers: [.option], expectedOutput: .keyDown, expectedIsMatched: true),
            ]
        )
    }
    ...
    func multipleModifiers_noBackslideActivation() throws {
        runScenario(
            ...
            steps: [
                ...
                ScenarioStep(time: 0.3, key: nil, modifiers: [.option, .command], expectedOutput: .keyDown, expectedIsMatched: true),
            ]
        )
    }
}
...
struct MouseClickTests {
    ...
    func mouseClick_discardsQuickModifierOnlyRecording() throws {
        ...
        #expect(startOutput == .keyDown)
        
        // Update time manually for next call
        processor = HotKeyProcessor(hotkey: HotKey(key: nil, modifiers: [.option]), minimumKeyTime: 0.15) {
            Date(timeIntervalSince1970: 0.25)
        }
        // Restore state (in a real test we'd keep the same processor instance, but here we are mocking time via closure)
        // Actually, since `now` is a closure stored in the struct, we can't easily change it after init unless it captures a mutable variable.
        // Let's refactor the test helper to handle this better.
    }
    
    // Refactored test helper for mouse clicks
    @Test
    func mouseClick_discardsQuickModifierOnlyRecording_Refactored() throws {
        var currentTime: TimeInterval = 0
        var processor = HotKeyProcessor(hotkey: HotKey(key: nil, modifiers: [.option]), minimumKeyTime: 0.15) {
            Date(timeIntervalSince1970: currentTime)
        }
    ...
    @Test
    func mouseClick_ignoredForKeyPlusModifier() throws {
        var currentTime: TimeInterval = 0
        var processor = HotKeyProcessor(hotkey: HotKey(key: .a, modifiers: [.command])) {
            Date(timeIntervalSince1970: currentTime)
        }
